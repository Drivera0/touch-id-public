...
print(f"exported touchid_housing_{_tag}.stl / .step to", out_dir)
print(f"  wall {wall_t}  cavity {lip_w - 2*wall_t:.2f} sq  screws M1.2 at {pcb_screw_pos}")
print(f"  ear screw: M2, unchanged, hole d{ear_hole_d}")
_face = body_w
_wall = (_face - sensor_window_d) / 2
_ceil = body_h - top_t
print(f"  sensor seat: window d{sensor_window_d:.2f} in a {_face:.2f} face"
      f" -> {_wall:.2f} mm of wall")
print(f"    ZW0905 flange d{sensor_module_d} on that face -> {(_face-sensor_module_d)/2:.3f} mm margin/side")
_barrel_bot = body_h - (sensor_thk - sensor_flange_t)   # flange sits ON the face
print(f"    barrel bottom at z {_barrel_bot:.2f}; cavity ceiling {_ceil:.2f};"
      f" U1 top 2.40 -> {_barrel_bot - 2.40:.2f} mm for the wires")
