"""
gen_frame_v2.py -- TouchID flashing jig FRAME v2 (printed part).

v2 = v1 with the Y-down/Y-up mirror fixed in b2f(), the orientation mark
moved onto the TOP face where you can actually see it, and TL/TR/BL/BR engraved into
the top face so orientation can never again be argued about in words.
...
Usage: python3 gen_frame_v2.py  ->  flash_frame_v2.stl + .step
"""
import os
...
def b2f(x, y):
    ...
    THE V1 BUG.  v1 returned (x - CCX, y - CCY), a pure translation.  KiCad
    is Y-DOWN and cadquery is Y-UP, so that fed a Y-down 2D coordinate
    straight into a Y-up 3D system and MIRRORED every Y-asymmetric feature
    of the frame: the opening, the seat bar and the orientation mark.  The
    printed v1 frame puts its window under the XIAO; all five pin tails and
    both dowels land on solid deck.  Y is negated here, and verify_frame_v2
    re-derives this mapping independently instead of importing it.
    """
    return (x - CCX, CCY - y)
...
# ---- CORNER LABELS on the TOP FACE --------------------------------------
# These DEFINE the canonical view: stand the part recess-up and turn it until
# the four labels read the right way round.  That view is "the top view", and
# TL/TR/BL/BR mean exactly what they say in it.  No convention to remember, no
# axis to get backwards, nothing to argue about in chat.
#
# They sit in the 5 mm top band over the outer wall, which is solid for the
# full 22 mm below, so a 0.6 mm engraving removes nothing structural.
LBL_H   = 3.0        # cap height
LBL_D   = 0.6        # engraved depth
LBL_X   = OUT_W/2 - 6.0
LBL_Y   = OUT_H/2 - 2.5
assert LBL_Y - LBL_H/2 > REC_H/2, "corner label must stay on the wall band"
for txt, sx, sy in (("TL", -1, +1), ("TR", +1, +1), ("BL", -1, -1), ("BR", +1, -1)):
    t = (cq.Workplane("XY", origin=(sx*LBL_X, sy*LBL_Y, H - LBL_D))
           .text(txt, LBL_H, LBL_D + 1.0, combine=False))
    base = base.cut(t)

# ---- ORIENTATION MARK -- NOW ON THE TOP FACE ----------------------------
#
# v1 put this triangle on the outer +Y wall, reasoning that a 5.0 mm top band
# could not carry an arrow without leaving 1.2-1.7 mm bands that wall_check.py
# flagged against JLC3DP's 2.0 mm minimum.  That flag was spurious and the v1
# comment half-admitted it: a 0.6 mm groove in a face backed by 21.4 mm of
# solid block creates no thin wall at all -- the material beside the groove is
# continuous with the whole part.  The TL/TR/BL/BR labels prove the point; they
# sit in the same band at the same depth.
#
# The cost of the side placement was real: you cannot see it looking straight
# down, which is exactly how you look at the jig while fitting the carrier.
# So the arrow moves onto the top band on the -Y side, pointing outward, in the
# clear span between the BL and BR labels.
MARK_D  = 0.6                       # engraved depth, same as the corner labels
MARK_W  = 14.0                      # width in X
MARK_Y0 = -(OUT_H/2 - 1.1)          # apex, 1.1 mm inside the outer edge
MARK_Y1 = -(REC_H/2 + 1.1)          # base, 1.1 mm outside the recess wall
assert MARK_W/2 + 2.0 < LBL_X - 2.6, "arrow must not run into the BL/BR labels"
mark = (cq.Workplane("XY", origin=(0, 0, H - MARK_D))
        .moveTo(0.0, MARK_Y0).lineTo(-MARK_W/2, MARK_Y1)
        .lineTo(MARK_W/2, MARK_Y1).close().extrude(MARK_D + 1.0))
base = base.cut(mark)


cq.exporters.export(base, os.path.join(OUT, "flash_frame_v2.stl"))
cq.exporters.export(base, os.path.join(OUT, "flash_frame_v2.step"))
...
print(f"  ARROW     {MARK_W} x {abs(MARK_Y0-MARK_Y1):.1f} triangle, {MARK_D} mm deep, "
      f"on the TOP face in the -Y band -- points at the DUT spacebar edge")
