...
Updated **2026-08-27, second pass**. Seven of the nine items are closed. Full
detail lives in `PART-LIBRARY.md` §9 and the two notes files.

### CLOSED

| | Was | Now |
|---|---|---|
| **B1** | MDBT50Q land pattern + keep-out unknown — blocked Task E | **Closed.** All 61 pads traced from Raytac Ver. K p.12 read at 400 %, then cross-checked against JLCPCB library `C5118826` pulled as JSON. The two frames agree to within a **constant offset of (0.4501, 0.5999) with a spread of 0.0002 mm across all 61 pads**. Keep-out: y 11.5 → 15.5 all layers (4.0 mm deep, ~1.0 overhang each side) plus a top-layer-only box 1.6 × 1.2 at x 2.95. `mdbt50q_footprint.py`, `crosscheck_mdbt50q.py` |
| **B2** | Cell straight to VDDH violates the 100 ms rise-time spec | **Mitigated.** `R7` ties BQ25505 **VBAT_OK to nRESET**, holding the CPU in reset until storage passes 3.47 V. Zero new parts. **Validate on the first board** — standard supervisor practice, unproven on this hardware |
| **B3** | LM66100 cannot be switched off by a GPIO | **Fixed.** U4 is now a **second TPS7A2033** with a logic-level EN, fed from VSTOR. Same footprint as U3, already traced. LM66100 deleted from the BOM |
| **B5** | Sensor rail headroom, dropout untraced | **Quantified.** TPS7A20 dropout is **140 mV max at 300 mA** (DQN, VOUT 2.5–5.5 V). Worst case, SENSOR_3.3V holds 3.0 V while VSTOR ≥ 3.14 V, so set the **firmware floor at 3.15 V**, not 3.0 |
| **B7** | REG0 DC/DC or LDO | **Settled: LDO.** Raytac Ver. K §8.2 block diagram p.41 shows **DCCH unconnected**. No 10 µH inductor, C11 deleted. Costs <5 % of the daily budget; board area wins |
| **B8** | 22 µH inductor unknown | **Identified.** `C2849435` = DMBJ **PNLS252012-220M** → 2.5 × 2.0 × **1.2 mm**. Height clears everything. **Land pattern still not traced** — see below |
| **B9** | VBAT_OK divider values unspecified | **Chosen.** ROK1 4.53M / ROK2 7.15M / ROK3 1.33M, sum 13.01 MΩ (TI window 11–15). Rising **3.47 V**, falling **3.12 V**, just above the cell's 3.0 V cut-off. Sign it off or change it |

### STILL OPEN

**B6 — the CP1254 has no protection module.** The safety one.
VARTA states on the face of the data sheet: *"Cell must not be used without
external safety electronics (PCM)."* BQ25505's VBAT_OV covers overcharge and
firmware covers undervoltage; neither covers a short, and neither is what VARTA
means. Decide: source the cell as a tabbed assembly with a PCM fitted, or add a
protection IC. Both cost board area or stack height that is not budgeted.

**B10 — three land patterns are still PROVISIONAL on pcb-v3.**

- **L1** — the DMBJ part is identified but its **land pattern was never traced**.
  An 0805 land stands in and is very likely too small. Get the DMBJ drawing.
- **BT1** — VARTA gives CP1254 dimensions "without Tags" and does not publish the
  tab geometry. Two generous wire pads are used instead. The cell is wired down
  from above anyway, so this is defensible either way.
- **C1, C2 (0603)** — IPC nominal, not traced to a specific part. 0402 and 0805
  come from Samsung's tables via DESIGN-SPEC §3.

**B11 — pcb-v3 is placed and checked but NOT ROUTED.** `check_connect` reports
27 split nets, which is the unrouted state. Route in KiCad, then re-run all
three checkers. Vias must avoid the pogo pads, which own B.Cu below y = −1.12.

**B12 — the cell is not retained from above,** and the Ø12.5 × 0.20 insulating
disc it now sits on is a new part that is not on the BOM.

---

## 7. Session log

### 2026-08-27, first pass (unattended) — Tasks A–D done, E blocked on B1

Part library traced; housing v5 built with all eight boolean checks at zero;
antenna orientation resolved (**+Y is the spacebar**, DESIGN-SPEC §2 was right
and §1's "+Y is KiCad-up" was the wrong half — now fixed); netlist written as an
executable self-check. Task E was **not started**, because U1 and L1 had no
verified land pattern and starting would have meant inventing one.

### 2026-08-27, second pass — B1 closed, Task E built

**B1 closed by cross-validation, not by squinting.** Raytac Ver. K p.12 was read
at 400 % in Chrome, then JLCPCB's library part `C5118826` was pulled as JSON from
easyeda.com's component API and unit-converted. The two agree on all 61 pads to
within a constant offset of (0.4501, 0.5999), **spread 0.0002 mm**. A slip on the
drawing would show as one pad off the constant; a bad library part as many.
Neither happened. `crosscheck_mdbt50q.py` re-proves it on demand.

Same trick settled the rest: `C882746` confirmed the BQ25505 land at 3.98 mm
against TI's 3.90, `C2849435` turned out to be a DMBJ **PNLS252012-220M**
(2.5 × 2.0 × 1.2), and Raytac §8.2 p.41 showed **DCCH unconnected** in LDO mode.

**Two architecture faults fixed.** The LM66100 is gone — its CE is a comparator
referenced to VIN and needs 3.38 V to switch off, which an nRF GPIO cannot reach,
so the sensor-standby saving never happened. U4 is now a second TPS7A2033 with a
logic-level EN. And `R7` ties VBAT_OK to nRESET so the CPU is held through the
slow VDDH ramp. Both use parts already traced; neither adds a new part number.

**pcb-v3 built.** 4 layers, 46 footprints, 177 pads, 7 zones.
`sexp_check` PARSE OK / 0 bare LF · `check_board` **0 violations** ·
`check_connect` 27 split, which is the unrouted state.

> **The finding that set the layout:** the BQ25505's land is 3.98 mm and each
> side strip beside a centred module is 3.72 mm. The charger does not fit either
> side. **U1 had to move off-centre to x −0.80.** The antenna still points at +Y.
> In Y it is the *housing*, not the board edge, that limits how close the antenna
> gets — the module is 15.5 long in a 17.94 cavity.

That shift then broke housing v5's cell ledges, because the cell no longer
overhangs the module on two sides. The cell now **rests on the module lid through
a Ø12.5 × 0.20 insulating pad** — simpler, and clearance improved from 1.41 to
**1.61 mm**. All eight boolean checks still return zero.

**Three checker bugs found and fixed along the way**, each the same lesson as
§7 trap 3 — a confident summary is not evidence:

- the clearance function used `min(dx, dy)` for overlapping boxes, reporting
  **−1.8 mm for a 0.2 mm overlap**;
- it skipped every same-footprint pad pair, which hid **BT1's own two wire pads
  0.100 mm apart** until `check_board` caught them;
- the bottom component rows were hardcoded at y −7.05 and did not follow U1 when
  it moved, putting a whole row under the module. They are derived now.

**Still open:** B6 (the cell has no protection module — the safety one), B10
(three provisional land patterns), B11 (**route it**), B12 (cell retention and
the insulating disc).

Nothing was ordered, priced, or added to a cart. `pcb-v2.kicad_pcb` and
`touchid_module_v4.py` are **byte-identical to the `pre-v3-rebuild` tag**.
