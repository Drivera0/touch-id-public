...
> [!important] Read [[touchid/DESIGN-SPEC|DESIGN-SPEC.md]] first
...
- [ ] **NEXT: run the pin test** to learn which slot pins carry power → see [[touchid/Pin Test Procedure|Pin Test Procedure]]
- [ ] Breadboard prototype with a XIAO ESP32-S3
- [ ] Custom PCB

## Current constraints + hardware decisions (2026-08-18)

- **Envelope:** module is ~**19 × 19 mm** square; everything fits *inside* (no overhang). Confirm by caliper measurement. → [[touchid/Mechanical and Housing|Mechanical and Housing]]
- **Power/data:** target is **wireless, powered from the keyboard** (flush, no cable/hole) — contingent on the pin test finding a usable power pin. → [[touchid/Architecture and Design|Architecture and Design]]
- **MCU:** ESP32-S3-MINI-1 is **too long (20.5 mm)**. Final wireless board uses **ESP32-C3-MINI-1 (13.2 × 16.6 mm)**; wired prototype uses a XIAO ESP32-S3 on breadboard. → [[touchid/Firmware and PCB|Firmware and PCB]]
- **Sensor:** small (`≤`~16 mm body) FPC-ribbon sensor, prefer `0xEF01` family (ZW/GROW) to keep firmware. Shortlist in [[touchid/Mechanical and Housing|Mechanical and Housing]].
- **PCB:** order fabbed **+ assembled (PCBA)** from JLCPCB/PCBWay — don't mill. → [[touchid/Firmware and PCB|Firmware and PCB]]
- **Housing:** FDM **PETG + brass heat-set inserts** (screw-durable), or SLA tough resin for detail. Print at UBC (HATCH Form 2 / ECE farm) — access gated. → [[touchid/UBC Facilities and Contacts|UBC Facilities and Contacts]]

## Notes in this folder

- [[touchid/Hardware Teardown|Hardware Teardown]] — what's inside the keyboard, the slot connectors (J4/J11), MCU, battery
- [[touchid/Pin Test Procedure|Pin Test Procedure]] — how to probe the 10 slot pins and record results
- [[touchid/Architecture and Design|Architecture and Design]] — the sidecar model, power, data path, Mac/Windows, SSH
- [[touchid/Firmware and PCB|Firmware and PCB]] — tinyTouch code review, improvements, chip choice, antenna, PCBA/assembly, atopile
- [[touchid/Mechanical and Housing|Mechanical and Housing]] — 19 mm fit constraint, the stack, sensor shortlist, housing material + printing
- [[touchid/UBC Facilities and Contacts|UBC Facilities and Contacts]] — scanner, printers, PCB, contacts, access rules
- [[touchid/Roadmap|Roadmap]] — the phased build plan
